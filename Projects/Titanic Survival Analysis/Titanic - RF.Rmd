...
tunegrid = expand.grid(min.node.size = nodesize, mtry = mtry, splitrule = split)

fit.cv = train(Survived ~ ., data = data.trn, method = "ranger",
               trControl = control2,
               tuneGrid = tunegrid)
...
```{r}
write.csv(test.final, "C:\\Users\\HP\\Desktop\\학교자료\\데이터프로젝트\\Kaggle - Titanic\\submission4.csv", row.names = F)
```